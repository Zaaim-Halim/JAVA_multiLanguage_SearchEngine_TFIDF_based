...
public class ArabicStemmer implements IStemmer{
    ...
    private static void initComponents()
    {
    	...
    	pathToStemmerFiles = new StringBuffer("resources/arabicStemmingFiles/").toString();
    	rootFound = false;
        stopwordFound = false;
        fromSuffixes = false;
        ...
        for( int i = 0; i < patterns.size ( ); i++ )
        {
            ...
            if ( pattern.length ( ) == word.length ( ) )
            {
                ...
                if ( word.length ( ) - 3 <= numberSameLetters )
                {
                    ...
                    modifiedWord = root.toString ( );
                    System.out.println("Befor ROOT: "+modifiedWord);
                    modifiedWord = isThreeLetters ( modifiedWord );
                    System.out.println("ROOT: "+modifiedWord);
                    if ( rootFound )
                    {
                        word = modifiedWord;
                        return word;
